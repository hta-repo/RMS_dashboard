...
//  UI
import { Box, useTheme, Stack, Grid, Backdrop, CircularProgress, Chip, Button } from "@mui/material";
...
const AllVouchers = () => {
    ...
    const allData = useCallback(() => {
        (async () => {
            ...
            const response = await GeneralService.getAllVoucherData({ page, limit });
    ...
    const columns = [
        ...
        },
        {
            ...
            flex: 1,
            valueGetter: (params) => {
                return CapitalWord(params.row?.ownerType ?? "");
            },
        },
        {
            field: "type",
            ...
            renderCell: ({ row }) => {
                return (
                    <Stack direction="row" spacing={2}>
                        ...
                        <EditIcon style={{ cursor: "pointer" }} onClick={() => navigate("/edit-vouchers/" + row.id)} />
                    </Stack>
                );
            },
        },
    ...
    const Delete = (id) => {
        (async () => {
            setLoader(true)
            const response = await GeneralService.deleteVoucher(id);
            if (response.data.success) {
                setError(true)
                setVariant('success')
            ...
            <Box m="20px">
                <Grid container spacing={2}>
                    ...
                    <Grid item xs={3} textAlign="right" mt="20px">
                        <Button type="button" color="secondary" variant="contained" sx={{ fontWeight: "bold" }} onClick={() => navigate("/create-vouchers")}>
                            Create Voucher
                        </Button>
                    </Grid>
                </Grid>
